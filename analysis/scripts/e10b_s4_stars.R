# E10b follow-up (Prompt E 2026-07-17): Scenario IV calibration inference
...
# Production protocol replicated: reps ascending from a cold session
# (matching b2_scenario4.R's own session structure), per rep:
# xs = standardize(gen_scenario4(seed_r)); set.seed(seed_r + 500000);
# run_methods_timed(xs); set.seed(seed_r + 300001);
...
write.csv(new_cal, file.path(OUTPUT_DIR, "e10b_s4_cal_new.csv"), row.names = FALSE)

old_cal <- read.csv(file.path(OUTPUT_DIR, "scenario4_calibration.csv"))
...
cat("TOTAL S4 STAR FLIPS:", tot_flips, "\n")
cat("E10B_S4_STARS_DONE\n")
