...
- **Statistical tests**: permutation inference with family-wise adjustment
- **Baselines**: center-only assessment to isolate information from widths
...
- **Behavior indices** (B_TC, B_RE, B_LC): range [-1, 1], 0 means balanced;
  positive values indicate intrusion-dominated embeddings

### 5. Using `dataSDA` Datasets

QAIDR does not redistribute example datasets. The converter below reads the
interval encodings used by the CRAN package `dataSDA`.
...
See the "Real Data Analysis" and "Simulation Study" vignettes for compact
tutorials. The complete seeded manuscript pipeline and frozen outputs are
documented in `README_REPRODUCIBILITY.md` in the GitHub repository.
