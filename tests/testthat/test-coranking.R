...
# --- Stage 3 repair: Q_RE range contract -------------------------------------
# Q_TC and Q_LC lie in [0, 1] unconditionally, but Q_RE is bounded above by 1
# only: the retained Lee & Verleysen (2009) normalizer H_K does not dominate the
# attainable weighted rank error at large K/n, so Q_RE can be slightly negative
# on valid input. Before the repair, .check_ranges warned that such a value
# "indicates an input or implementation problem". It must not.

test_that("negative Q_RE at large K/n is reported, not flagged as an error", {
  K <- 4L
  x <- c(Q_TC = 0.5, B_TC = 0, Q_RE = -1e-05, B_RE = 0, Q_LC = 0.5, B_LC = 0)

  # no warning: a small negative Q_RE is a property of the published normalizer
  expect_silent(suppressMessages(QAIDR:::.check_ranges(x, K)))

  # but it is still surfaced to the user as a message
  expect_message(QAIDR:::.check_ranges(x, K), "Q_RE")
})

test_that("Q_RE above 1 is still an error, and Q_TC/Q_LC keep the [0,1] floor", {
  K <- 4L
  base <- c(Q_TC = 0.5, B_TC = 0, Q_RE = 0.5, B_RE = 0, Q_LC = 0.5, B_LC = 0)

  bad_hi <- base; bad_hi[["Q_RE"]] <- 1.5
  expect_warning(QAIDR:::.check_ranges(bad_hi, K), "Q_RE")

  bad_tc <- base; bad_tc[["Q_TC"]] <- -0.1
  expect_warning(QAIDR:::.check_ranges(bad_tc, K), "Q_TC")

  bad_lc <- base; bad_lc[["Q_LC"]] <- -0.1
  expect_warning(QAIDR:::.check_ranges(bad_lc, K), "Q_LC")
})
